...
ifdef INTERACTIVE
define DOCKER
	docker run -it --rm -e GOCACHE=/app/.cache -e IN_DOCKER=1 --user $(shell id -u):$(shell id -g) -w /app -v $(PWD):/app -v $(GOPATH):/go --init $(1) /bin/bash -c '$(2)'
endef
else
define DOCKER
...
ifeq ($(MINIKUBE_BUILD_IN_DOCKER),y)
	...
	if [ ! -d $(BUILD_DIR)/buildroot ]; then \
		...
		sh -c "cat deploy/iso/minikube-iso/0001-patch-building-arm64.patch | (cd $(BUILD_DIR)/buildroot; git apply)"; \
	fi;
